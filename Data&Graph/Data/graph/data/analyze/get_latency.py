    ...
    try:
        for i in range(max_count):
            with open(os.path.join(config.DataPath, path, str(i) + ".json"), "r") as fileR:
                ...
                datas.append((data["tag"], data["model_name"], data["total_cost_by_ms"]))
    except exception as ex:
        print("warning: count(%s) < %d, but we had deal data that found (=%d)." % (path, max_count, len(datas)))

    with open(os.path.join(config.AimFold, path, "latency.csv"), "w") as fileW:
        print("id, model-name, latency", file=fileW)
        for data in datas:
            print(data[0], ",", data[1], ",", data[2], file=fileW)

    datas.sort(key=lambda value: value[2])
    with open(os.path.join(config.AimFold, path, "latency_sort.csv"), "w") as fileW:
        print("id, model-name, latency", file=fileW)
        for data in datas:
            print(data[0], ",", data[1], ",", data[2], file=fileW)
