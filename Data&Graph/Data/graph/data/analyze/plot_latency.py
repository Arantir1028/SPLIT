...
def PlotAccumulates(benchs: list, envs: list):
    ...
    for env in envs:
        ...
        for bench in benchs:
            ...
            blocks= [[[""],["total"]]]
            info={}
            with open(os.path.join(config.AimFold,bench,env,"statistic_latency.json"),"r") as fp:
                info=json.load(fp)
            ...
            plt.savefig(os.path.join(config.AimFold,bench,env,"accumulate_latency.svg"), dpi=300,format="svg")
            plt.close()
        ...
        plt.savefig(os.path.join(config.AimFold,"total",env,"accumulate_latency.svg"), dpi=300,format="svg")
        plt.close()
    ...
    for env in envs:
        ...
        for bench in benchs:
            ...
            for idx,name in enumerate(info):
                plt.bar(xs+bar_width*idx,np.array([info[name]["min"], info[name]["avg"], info[name]["max"]]), width=bar_width, label=name)
            ...
            plt.title(config.GetDescribe(env))
            os.makedirs(os.path.join(config.AimFold,bench,env),exist_ok=True)
            plt.savefig(os.path.join(config.AimFold,bench,env,"range_latency.svg"), dpi=300,format="svg")
            plt.close()
            ...
            plt.ylabel('Std. Deviation of Latency')
            plt.title(config.GetDescribe(env))
            os.makedirs(os.path.join(config.AimFold,bench,env),exist_ok=True)
            plt.savefig(os.path.join(config.AimFold,bench,env,"std_latency.svg"), dpi=300,format="svg")
            plt.close()
        ...
        plt.axhline(y=total_data["DLIR"]["avg"], color='r', linestyle='--', linewidth=0.5)
        plt.xticks(np.arange(len(total_data)), list(total_data.keys()))

        plt.ylabel('Average Latency')
        plt.title(config.GetDescribe(env))
        os.makedirs(os.path.join(config.AimFold,"total",env),exist_ok=True)
        plt.savefig(os.path.join(config.AimFold,"total",env,"total_avg_latency.svg"), dpi=300,format="svg")
        plt.close()
        ...
        plt.axhline(y=total_data["DLIR"]["max"], color='r', linestyle='--', linewidth=0.5)
        plt.xticks(np.arange(len(total_data)), list(total_data.keys()))

        plt.ylabel('99% Latency')
        plt.title(config.GetDescribe(env))
        os.makedirs(os.path.join(config.AimFold,"total",env),exist_ok=True)
        plt.savefig(os.path.join(config.AimFold,"total",env,"total_max_latency.svg"), dpi=300,format="svg")
        plt.close()
        ...
        plt.title(config.GetDescribe(env))
        os.makedirs(os.path.join(config.AimFold,"total",env),exist_ok=True)
        plt.savefig(os.path.join(config.AimFold,"total",env,"total_std_latency.svg"), dpi=300,format="svg")
        plt.close()
