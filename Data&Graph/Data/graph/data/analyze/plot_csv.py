...
def PlotAccumulates(benchs: list, envs: list):
    ...
    for env in envs:
        ...
        for bench in benchs:
            ...
            with open(os.path.join(config.AimFold,bench,env,"accumulate_latency_.csv"), "w") as fp:
                ...
                for i in range(len(xs)):
                    for j in range(len(labels)):
                        fp.write(str(xs[i]) + "," + str((ys_list[j]+[ys_list[j][-1]]*(len(xs)-len(ys_list[j])))[i]))
                        if j < len(labels) - 1:
                            fp.write(",")
                        else:
                            fp.write("\n")
                ...
                #     print(xs, file=fp)
                #     print(ys_list[i]+[ys_list[i][-1]]*(len(xs)-len(ys_list[i])), file=fp)
        ...
        with open(os.path.join(config.AimFold,"total",env,"accumulate_latency_.csv"), "w") as fp:
            for i in range(len(total_labels)):
                fp.write("x," + total_labels[i])
                if i < len(total_labels) - 1:
                    fp.write(",")
                else:
                    fp.write("\n")
            for i in range(len(total_xs)):
                for j in range(len(total_labels)):
                    fp.write(str(total_xs[i]) + "," + str((total_ys_list[j] + [total_ys_list[j][-1]] * (len(total_xs) - len(total_ys_list[j])))[i]))
                    if j < len(total_labels) - 1:
                        fp.write(",")
                    else:
                        fp.write("\n")
...
def PlotRange(benchs: list, envs: list):
    ...
    for env in envs:
        ...
        for bench in benchs:
            ...
            with open(os.path.join(config.AimFold,bench,env,"range_.csv"), "w") as fp:
                print("name,min,avg,max", file=fp)
                for idx,name in enumerate(info):
                    print(name + "," + str(info[name]["min"]) + "," + str(info[name]["avg"]) + "," + str(info[name]["max"]), file=fp)
            ...
            with open(os.path.join(config.AimFold,bench,env,"std_.csv"), "w") as fp:
                print("name,Std. Deviation of RR", file=fp)
                for i in range(len(info)):
                    print(str((list(info.keys()))[i]) + "," + str(avgs[i]), file=fp)
        ...
        with open(os.path.join(config.AimFold,"total",env,"total_avg_max_.csv"), "w") as fp:
            print("name,Average RR, Max RR", file=fp)
            for i in range(len(total_data)):
                print(str((list(total_data.keys()))[i]) + "," + str(avgs[i]) + "," + str(maxs[i]), file=fp)
        ...
        # with open(os.path.join(config.AimFold,"total",env,"total_max_.csv"), "w") as fp:
        #     print("name,Max RR", file=fp)
        #     for i in range(len(total_data)):
        #         print(str((list(total_data.keys()))[i]) + "," + str(maxs[i]), file=fp)
        ...
        with open(os.path.join(config.AimFold,"total",env,"total_std_.csv"), "w") as fp:
            print("name,Std. Deviation of RR", file=fp)
            for i in range(len(total_data)):
                print(str((list(total_data.keys()))[i]) + "," + str(avgs[i]), file=fp)
