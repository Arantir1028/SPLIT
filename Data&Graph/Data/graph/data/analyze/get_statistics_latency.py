...
def main():
    for bench_fold in config.BenchFoldNames:
        for env in config.RunEnvs:
            datas=load_anff_from_csv(str(os.path.join(bench_fold, env, "latency.csv")), ignore_head=True)
            ...
            with open(os.path.join(config.AimFold,bench_fold,env,"statistic_latency.json"),"w") as fp:
